...
DROP POLICY IF EXISTS "ads_public_read" ON public.ads; CREATE POLICY "ads_public_read" ON public.ads FOR SELECT USING (true);
DROP POLICY IF EXISTS "ads_public_manage" ON public.ads; CREATE POLICY "ads_public_manage" ON public.ads FOR ALL USING (true) WITH CHECK (true);
CREATE OR REPLACE FUNCTION public.increment_ad_click(ad_id UUID) RETURNS void LANGUAGE sql SECURITY DEFINER AS $$ UPDATE public.ads SET clicks=clicks+1 WHERE id=ad_id; $$;
